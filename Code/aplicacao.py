...
def data_pre_process(df_clean, remove_outliers, outliers_method, fix_imbalance, normalize, normalize_method, mlflow_db, experiment_id, experiment_name, target_col, file_to_load, file_name, save_format):
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=experiment_name):
        data_exp = pc.setup(df_clean, target="shot_made_flag", remove_outliers=remove_outliers, outliers_method=outliers_method, fix_imbalance=fix_imbalance, normalize=normalize, normalize_method=normalize_method)
...
def eda_tab():
    ...
    file_to_load = st.sidebar.file_uploader("Carregar arquivo de dados:", type=["csv", "parquet"])
    if file_to_load is not None:
        ...
        selected_columns = st.sidebar.multiselect("Selecionar colunas para análise:", options=sorted(df.columns.tolist()), default=sorted(df.columns.tolist()))

        st.sidebar.subheader("Parâmetros(Dados)")
        # Realizar a limpeza de dados
        if st.sidebar.toggle("Remover dados faltantes(null/NA):"):
            df_clean = df[selected_columns].dropna()
        else:
            df_clean = df[selected_columns]

        # Imbalance
        fix_imbalance = st.sidebar.toggle("Balancear(SMOTE) ")
        
        # Outliers
        remove_outliers = st.sidebar.toggle("Remover 'Outliers'")  
        if remove_outliers:
        ...
        # Normalize
        normalize = st.sidebar.toggle("Normalizar")    
        if normalize:
        ...
        experiment_id = st.sidebar.text_input("ID do experimento avaliação:", value="Kob" )
        experiment_name = st.sidebar.text_input("Nome do experimento avaliação:", value="PreparacaoDados")
        mlflow_db = st.sidebar.text_input("Nome do banco de dados:", "mlruns_kob")
...
def train_model_tab():
    ...
    mlflow_db = st.sidebar.text_input("Nome do banco de dados:", f"{mlflow_db}")
    experiment_id = st.sidebar.text_input("ID do experimento:", f"{experiment_id}")
    experiment_name = st.sidebar.text_input("Nome do experimento:", "Treinamento")

    # Classification Plots with Popover
    st.sidebar.subheader("Plots de Classificação")
    selected_plots = st.sidebar.multiselect("Plots a serem gerados:", options=list(plots_map_class.keys()), format_func=lambda x: x)
...
def avaliacao_modelo_tab():
    ...
    file_to_load  = st.sidebar.file_uploader("Carregar dataset de produção (CSV ou Parquet):", type=["csv", "parquet"])
    if file_to_load is not None:
    ...
    experiment_name = st.sidebar.text_input("Nome do experimento avaliação:", value=f"Avaliação_{experiment_name}")    
    mlflow_db = st.sidebar.text_input("Nome do banco de dados do MLflow:", value=mlflow_db)    
...
def producao_tab():
    ...
    mlflow_db = st.sidebar.text_input("Nome do banco de dados(MLflow):", "mlruns_kob")
    model_uri = st.sidebar.text_input("Caminho do Modelo(Uri MLflow):", "")
    if st.sidebar.button("Aplicar Modelo"):
        report, acuracia = production_model(df_producao, columns, mlflow_db, target_col, model_uri)
...
def production_model(df_producao, columns, mlflow_db, target_col, model_uri):
    import os
    import pandas as pd
    import mlflow
    import streamlit as st
